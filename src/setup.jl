...
function ReadPDB(pdbName)
    ...
    resnr = []
    elem = []
    df = 1 / 10  # factor from converting between Angstrom and nm
...
function ReadTopFile(top_file::AbstractString)
            ...
            if occursin("#include", line)
                ...
                open(filename) do file
                    for lins in eachline(file)
                        ...
                        if occursin("[ moleculetype ]", lines)
                            molNumber += 1
                        end
                    end
                end
    ...
    if molNumber != molNumberCheck
        error(
            "The number of moleculetypes in topology file ",
            top_file,
    ...
    open(top_file) do file
        ...
        for lin in eachline(file)
            ...
            if startswith(line, '[') && endswith(line, ']')
                if ((zone == "moleculetype") || (zone == "system")) &&
                   (length(topatoms) > 0)
                    push!(
                        molparams,
                        MolParam(
                            topName,
                            topnrexcl,
                            [topatoms[i] for i = 1:length(topatoms)],
                            [topbonds[i] for i = 1:length(topbonds)],
                            [topangles[i] for i = 1:length(topangles)],
                            [topdihedrals[i] for i = 1:length(topdihedrals)],
                        ),
                    )
                    topatoms = Atoms[]
                    topbonds = Bonds[]
                    ...
                    topdihedrals = Dihedrals[]
                    molType += 1
                elseif ((zone == "moleculetype") || (zone == "system")) &&
                       length(topatoms) == 0
                    molType += 1
                end
                zone = strip(line[2:end-1])
            ...
            elseif zone == "atomtypes" && length(split(line)) > 2
                ...
                push!(
                    ...
                    Atomtypes(
                        inline[1],
                        inline[2],
                        parse(Float64, inline[3]),
                        parse(Float64, inline[4]),
                        inline[5],
                        parse(Float64, inline[6]),
                        parse(Float64, inline[7]),
                    ),
                )
            ...
            elseif occursin("#include", line)
                ...
                println(
                    "Reading Topology information from itp file for ",
                    filename,
                )
                open(filename) do file
                    ...
                    for itplin in eachline(file)                # read each line of file
                        ...
                        if itpzone == "moleculetype" &&
                           length(split(itpzone)) >= 1#Make new molecule object
                        ...
                        elseif itpzone == "angles" && length(split(itpline)) > 2
                            ...
                            push!(
                                ...
                                Angles(
                                    ...
                                    parse(Int64, s[3]),
                                    parse(Int64, s[4]),
                                    deg2rad(parse(Float64, s[5])),
                                    parse(Float64, s[6]),
                                ),
                            )

                        elseif itpzone == "dihedrals" &&
                               length(split(itpline)) > 2
                    ...
                    push!(
                        ...
                        MolParam(
                            itpName,
                            itpnrexcl,
                            [itpatoms[i] for i = 1:length(itpatoms)],
                            [itpbonds[i] for i = 1:length(itpbonds)],
                            [itpangles[i] for i = 1:length(itpangles)],
                            [itpdihedrals[i] for i = 1:length(itpdihedrals)],
                        ),
                    )
                end # closes itp file
    ...
    systemTopology = FFParameters(
        defaults,
        [atomtypes[i] for i = 1:length(atomtypes)],
        [molparams[i] for i = 1:length(molparams)],
        system,
        molecules,
    )
...
function MakeAtomArrays(
    ...
    # Check that all atoms are in a positive box. Sometimes the box is centered at 0,
    ...
    # right, not carry on.
    for i = 1:length(atomsPDB.r)
        mol = FindMolType(String(atomsPDB.resnm[i]), molecule_list)
        atom = FindAtomInMol(String(atomsPDB.atomnm[i]), mol, systemTop)
        ...
        pArray[i] = ParticleAtom(
            atomsPDB.resnr[i],
            mol,
            atomTypeNumber,
            mass,
            SVector{3}([atomsPDB.r[i]...]),
            zeros(SVector{3}), # velocity
            zeros(SVector{3}), # force
            charge,
        )
    end
    ##############################
    ...
    mass = []
    start = 1
    for (i, atom) in enumerate(pArray)
        if i == 1
            push!(COM, atom.r)
            push!(mass, atom.mass)
        elseif atom.molNum == pArray[i-1].molNum
            # this atom is in the same molecule as previous atom, append
            push!(COM, atom.r)
            push!(mass, atom.mass)
        else
            # this starts a new molecule, complete molecule, start new one
            mcount += 1
            centerOfMass = SVector(Center_of_Mass(COM, mass)...)
            mol = FindMolType(String(atomsPDB.resnm[i-1]), molecule_list)
            molTracker[mcount] = Molecule(
                start,
                i - 1,
                centerOfMass,
    ...
    #     mass=[]
    #     start = iter - 1
    #     if pArray[iter].molNum != pArray[iter-1].molNum
    #         # molecule has 1 atom in it
    #         println("$iter, $mcount, $acount, $(pArray[iter].molNum)")
    #         acount += 1
    #         push!(COM,pArray[iter-1].r)
    #         push!(mass,pArray[iter-1].mass)
    #         if  pArray[iter-1] == pArray[end] break end # break if we are at the end of the array
    #     else
    #         # molecule has more than 1 atom in it
    ...
    #
    #     ed = iter - 1
    #     mol = FindMolType(String(atomsPDB.resnm[acount]), molecule_list )
    #     molTracker[mcount] = Molecule( start, ed, centerOfMass,zeros(SVector{4, Float64}),
    #                                     0.0, mol
    #                                 )
    #     iter += 1
    #
    # end
    # Convert from Array of structs to a Table. rows are each atom, columns
    # are each field. so, a column contains for instance, all velocities.
    # StructArray columns can be isbits true i.e., good to go on GPU
...
function MakeTables(systemTop::FFParameters, atomsPDB::Topology, molecule_list)
    ...
    for i = 1:(length(atomsPDB.resnr)-1) # holds last molecules index
        ...
        molNum = FindMolType(String(atomsPDB.resnm[i]), molecule_list)   # Int64
        ...
        for j = 1:length(systemTop.molParams[molNum].dihedrals)
            deepCopyDihedral =
                deepcopy(systemTop.molParams[molNum].dihedrals[j])
    ...
    intraFF = IntraForceField(
        [IntraBond[i] for i = 1:length(IntraBond)],
...
function readNIST(filename)
    ...
    for m = 1:L
        SoA[m] = Molecule(
            ...
            [
                SVector(QQcoords[i][1], QQcoords[i][2], QQcoords[i][3])
                for i = indx:(indx+2)
            ],
        )
    ...
    SoArray[length(charge)+1] = Particle(1, 2, SVector(0.0, 0.0, 0.0), 0.0)
...
function OverflowTable(vdwTable, qqTable, kappa)
    ...
    @inbounds for i = 1:num_atom_types
        @inbounds for j = 1:num_atom_types
            ...
            if abs(ϵ) > 0.00 && abs(σ) > 0.00 # the abs() is overkill and unphysical
                ...
                push!(
                    logFile,
                    "Newton result for interaction: $i - $j $(LJCutoff²[i,j]) ",
                )
            end
        end
    end
    ...
    len = size(LJCutoff², 1)
    overflowCutoff² = OverFlowTable(
        [LJCutoff²[i, j] for i = 1:len, j = 1:len], # located in structs.jl
