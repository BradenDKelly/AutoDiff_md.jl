...
function MakeAtomArrays(systemTop::FFParameters,
    ...
    for i in 1:length(atomsPDB.r)
        ...
        pArray[i] = ParticleAtom( atomsPDB.resnr[i],
                                mol,
                                atomTypeNumber,
                                mass,
    ...
    for (i, atom) in enumerate(pArray)
        ...
        elseif atom.molNum == pArray[i-1].molNum
            ...
            molTracker[mcount] = Molecule(start, i - 1, centerOfMass,
    ...
    molTracker[mcount+1] = Molecule(start, pArray[end].molNum, centerOfMass,
                                zeros(SVector{4, Float64}),
                                0.0, mol
                                )
...
function MakeTables(systemTop::FFParameters,
                    atomsPDB::Topology,
                    molecule_list)
